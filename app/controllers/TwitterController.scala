...
object TwitterController extends Controller {
  ...
  /**
   * Create a new tweet in memory db
  ...
  def create = Action(parse.json) {
    request =>
     val tweet = request.body.as[Tweet]
      ...
      Ok(toJson("created tweet " + tweet.status))

  }

  /**
   * List all of the current tweets in the memory db
   * @return
   */
...
/**
  ...
  def parseAndSaveTweet(json: JsValue, owner: String, saveTweetInDb: Boolean): Unit = {
    ...
    Tweet.findByTwitterId(tweet.twitterId) match {
      case Some(t) => Logger.debug("tweet already exists, not saving")
      case _ => {
        Logger.info("Adding tweet from " + tweet.user)
        Tweet.create(tweet)
